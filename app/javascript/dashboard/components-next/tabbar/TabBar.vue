...
<template>
  <div
    class="relative flex items-center h-9 rounded-xl bg-n-surface-1 border border-n-weak dark:bg-n-solid-1 w-fit transition-all duration-200 ease-out has-[button:active]:scale-[1.01]"
  >
    <div
      class="absolute rounded-xl bg-n-background shadow-sm pointer-events-none h-8 outline-1 outline outline-n-weak inset-y-0 top-0.5"
    ...
    <template v-for="(tab, index) in tabs" :key="index">
      <button
        ...
        class="relative z-10 px-4 truncate py-1.5 text-sm border-0 outline-1 outline-transparent rounded-xl transition-all duration-200 ease-out hover:text-n-slate-12 active:scale-[1.02]"
        :class="[
          activeTab === index
