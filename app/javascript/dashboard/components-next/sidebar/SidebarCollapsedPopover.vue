...
<template>
  <TeleportWithDirection>
    <div
      ref="popoverRef"
      class="fixed z-[100] min-w-[220px] max-w-[300px]"
      :style="{
    ...
    >
      <div
        class="bg-[rgb(var(--surface-1))] border border-[rgba(var(--slate-12),0.08)] w-72 rounded-[14px] shadow-[0_8px_24px_rgba(15,23,42,0.08)] py-2.5 px-2.5"
      >
        <div
          class="px-3 py-2 text-[9px] font-semibold text-n-slate-10 uppercase tracking-[0.12em] border-b border-[rgba(var(--slate-12),0.06)] mb-2"
        >
          {{ label }}
        </div>
        <ul
        ...
        >
          <template v-for="child in accessibleChildren" :key="child.name">
            ...
            <li v-if="child.children" class="py-0.5">
              <button
                class="flex items-center gap-2 px-3 py-2 w-full rounded-[10px] text-n-slate-11 hover:bg-[rgba(var(--sage-accent),0.14)] transition-all duration-150 ease-out text-left rtl:text-right"
              ...
              <Transition v-bind="transition">
                ...
                >
                  ...
                  >
                    <button
                      class="flex items-center gap-2 px-3 py-2 w-full rounded-[10px] text-sm text-left rtl:text-right transition-all duration-150 ease-out"
                      :class="{
                        'text-n-slate-12 bg-[rgba(var(--sage-accent),0.24)]':
                          isActive(subChild),
                        'text-n-slate-11 hover:bg-[rgba(var(--sage-accent),0.14)]':
                          !isActive(subChild),
                      }"
