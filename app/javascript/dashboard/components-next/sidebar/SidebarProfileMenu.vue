...
<template>
  ...
  >
    <template #trigger="{ toggle, isOpen }">
      <button
        class="flex gap-3 items-center p-2.5 text-left rounded-[12px] cursor-pointer transition-all duration-150 ease-out bg-white border border-[rgba(var(--slate-12),0.06)] hover:border-[rgba(var(--slate-12),0.1)]"
        :class="[
          {
            'bg-white border-[rgba(var(--slate-12),0.1)]': isOpen,
          },
      ...
      >
        ...
        <div v-if="!isCollapsed" class="min-w-0">
          <div class="text-[12px] font-semibold leading-4 truncate text-n-slate-12 tracking-[-0.005em]">
            {{ currentUser.available_name }}
          </div>
          <div class="text-[10px] truncate text-n-slate-10">
            {{ currentUser.email }}
          </div>
        </div>
      </button>
