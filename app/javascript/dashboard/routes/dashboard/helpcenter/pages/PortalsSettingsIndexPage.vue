...
const { isOnLumeCloud } = useAccount();
...
const handleUpdatePortalConfiguration = portalObj => {
  ...
  // If custom domain is added or updated, fetch SSL status after a delay of 5 seconds (only on Lume cloud)
  if (portalObj?.custom_domain && isOnLumeCloud.value) {
    setTimeout(() => {
      fetchSSLStatus();
