...
<template>
  ...
  >
    ...
    <p class="text-sm font-normal text-n-slate-11">
      ...
      <span v-if="isOnLumeCloud && !isAdmin">
        {{ $t('GENERAL_SETTINGS.LIMIT_MESSAGES.NON_ADMIN') }}
      </span>
      <span v-else-if="!isOnLumeCloud && !isSuperAdmin">
        {{ $t(`${featurePrefix}.ENTERPRISE_PAYWALL.ASK_ADMIN`) }}
      </span>
    </p>
    <template v-if="isOnLumeCloud && isAdmin">
    ...
    <template v-else-if="!isOnLumeCloud && isSuperAdmin">
