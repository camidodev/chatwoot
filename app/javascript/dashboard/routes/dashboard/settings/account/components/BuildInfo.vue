...
const latestLumeVersion = computed(() => {
  return currentAccount.value.latest_Lume_version;
});

const globalConfig = useMapGetter('globalConfig/get');

const hasAnUpdateAvailable = computed(() => {
  if (!semver.valid(latestLumeVersion.value)) {
    return false;
  }

  return semver.lt(globalConfig.value.appVersion, latestLumeVersion.value);
});
...
<template>
  <div class="p-4 text-sm text-center">
    <div v-if="hasAnUpdateAvailable && globalConfig.displayManifest">
      {{
        t('GENERAL_SETTINGS.UPDATE_Lume', {
          latestLumeVersion: latestLumeVersion,
        })
      }}
    </div>
