...
const COLOR_SCHEMES = {
  blue: [
    'bg-n-blue-3/55 border border-n-blue-4/20',
    'bg-n-blue-4/70 border border-n-blue-5/20',
    'bg-n-blue-5/80 border border-n-blue-6/20',
    'bg-n-blue-6/85 border border-n-blue-7/20',
    'bg-n-blue-7/90 border border-n-blue-8/20',
    'bg-n-blue-8/95 border border-n-blue-9/20',
  ],
  green: [
    'bg-[rgba(var(--sage-accent),0.18)] border border-[rgba(var(--sage-accent),0.25)]',
    'bg-[rgba(var(--sage-accent),0.3)] border border-[rgba(var(--sage-accent),0.32)]',
    'bg-[rgba(var(--sage-accent),0.45)] border border-[rgba(var(--sage-accent),0.4)]',
    'bg-[rgba(var(--sage-accent),0.58)] border border-[rgba(var(--sage-accent),0.46)]',
    'bg-[rgba(var(--sage-accent),0.72)] border border-[rgba(var(--sage-accent),0.54)]',
    'bg-[rgba(var(--sage-accent),0.88)] border border-[rgba(var(--sage-accent),0.62)]',
  ],
};
...
const getHeatmapLevelClass = useMemoize(
  (value, quantileRangeArray, colorScheme) => {
    if (!value)
      return 'border border-[rgba(var(--slate-12),0.05)] bg-[rgba(var(--slate-12),0.025)] dark:bg-n-slate-1/30';
    ...
    <template v-if="isLoading">
      <div class="grid gap-[5px] flex-shrink-0">
        <div
          ...
          :key="ii"
          class="w-full rounded-md bg-[rgba(var(--slate-12),0.04)] dark:bg-n-slate-1 animate-loader-pulse h-8 min-w-[70px]"
        />
      </div>
      <div class="grid gap-[5px] w-full min-w-[700px]">
        ...
        >
          <div
            v-for="jj in 24"
            :key="jj"
            class="w-full h-8 rounded-md bg-[rgba(var(--slate-12),0.04)] dark:bg-n-slate-1 animate-loader-pulse"
          />
        </div>
      </div>
      <div />
      ...
      >
        ...
        >
          <div
            ...
            :key="data.timestamp"
            class="h-8 rounded-md cursor-pointer"
