...
export default {
  components: { Banner },
  props: {
    latestLumeVersion: { type: String, default: '' },
  },
  setup() {
  ...
  computed: {
    ...
    updateAvailable() {
      return hasAnUpdateAvailable(
        this.latestLumeVersion,
        this.globalConfig.appVersion
      );
    },
    bannerMessage() {
      return this.$t('GENERAL_SETTINGS.UPDATE_Lume', {
        latestLumeVersion: this.latestLumeVersion,
      });
    },
    shouldShowBanner() {
      return (
        ...
        !this.isVersionNotificationDismissed(this.latestLumeVersion) &&
        this.isAdmin
      );
    },
  },
  methods: {
    ...
    dismissUpdateBanner() {
      ...
      if (updatedDismissedItems instanceof Array) {
        updatedDismissedItems.push(this.latestLumeVersion);
      } else {
        updatedDismissedItems = [this.latestLumeVersion];
      }
      LocalStorage.set(
...
<template>
  <Banner
    ...
    href-link="https://github.com/Lume/Lume/releases"
