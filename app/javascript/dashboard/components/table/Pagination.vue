...
<template>
  <div class="flex items-center justify-between rounded-[18px] border border-[rgba(var(--slate-12),0.06)] bg-white px-4 py-3 shadow-[0_6px_18px_rgba(15,23,42,0.03)]">
    <div class="flex flex-1 items-center gap-3 justify-between">
      <p class="text-sm truncate text-n-slate-10 mb-0">
      ...
      <div class="flex items-center gap-2">
        <FilterSelect
          ...
          variant="outline"
          hide-icon
          class="[&>button]:text-n-slate-11 [&>button]:hover:text-n-slate-12 [&>button]:h-8 [&>button]:rounded-full [&>button]:border-[rgba(var(--slate-12),0.06)] [&>button]:bg-[rgba(var(--sage-accent),0.14)]"
          :options="pageSizeOptions"
        />
        <nav class="isolate inline-flex items-center gap-1">
          <Button
            icon="i-lucide-chevrons-left"
            ghost
            slate
            sm
            class="!size-8 !rounded-full !border !border-[rgba(var(--slate-12),0.06)] !bg-white"
          ...
          <Button
            ...
            xs
            outline
            :color="page == currentPage ? 'slate' : 'slate'"
            class="!h-8 min-w-8 !rounded-full !border !border-[rgba(var(--slate-12),0.06)] !bg-white"
            @click="table.setPageIndex(page - 1)"
          >
            <span
              class="text-center"
              :class="{
                'text-n-slate-12 font-medium': page == currentPage,
                'text-n-slate-10': page != currentPage,
              }"
            >
              {{ page }}
            </span>
